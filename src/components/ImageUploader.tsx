...
interface ImageUploaderProps {
  ...
  labelClass?: string;
  inputClass?: string;
  previewClass?: string; // Added this property
}
...
export function ImageUploader({
  ...
  labelClass,
  inputClass,
  previewClass, // Destructured this property
}: ImageUploaderProps) {
      ...
      {value && (
        <div className="space-y-2">
          ...
          <button
            type="button"
            onClick={() => onChange("")}
            className="text-xs text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors flex items-center justify-center gap-1 w-full"
          >
            <X size={12} />
            Clear image
          </button>
