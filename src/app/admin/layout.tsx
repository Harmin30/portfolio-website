  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white dark:bg-[#0f1115]">
        ...
        />
        <span className="mt-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Loading</span>
      </div>
    );
  }
    ...
    <div className="flex h-screen bg-slate-50/50 dark:bg-[#0f1115] text-slate-900 dark:text-slate-100">
      ...
      `}>
        ...
        <div className="h-20 flex items-center px-8 border-b border-slate-100 dark:border-white/[0.05]">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-sm tracking-tight">Core<span className="text-indigo-500 font-medium">Admin</span></span>
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">Management</span>
            </div>
          </Link>
        </div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          <p className="px-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] mb-4">Nav Menu</p>
          {adminLinks.map((link) => {
            ...
            return (
              ...
              >
                <link.icon size={17} className={isActive ? "text-indigo-500" : "text-slate-400 group-hover:text-slate-600"} />
                <span className="flex-1">{link.label}</span>
                {isActive && (
                  <motion.div layoutId="nav-pill" className="w-1 h-5 bg-indigo-500 rounded-full" />
                )}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-white/[0.05] bg-slate-50/30 dark:bg-black/10">
          <div className="flex items-center gap-3 px-4 py-3 mb-2 rounded-2xl bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.05]">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] text-slate-400 font-medium truncate uppercase tracking-tighter">Admin</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-sm font-bold text-slate-800 dark:text-slate-100 tracking-tight leading-none mb-1">
                {adminLinks.find(l => l.href === pathname)?.label || "Dashboard"}
              </h1>
