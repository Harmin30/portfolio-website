...
/* --- THE ULTIMATE PROFESSIONAL MAPPING --- */
            ...
            {categories.map((cat) => (
              ...
              >
                <div className="relative z-10 flex items-center justify-between gap-4">
                  <span
                    className={`text-[10px] md:text-[11px] uppercase tracking-[0.2em] font-black transition-all ${activeCat === cat ? "text-blue-600" : "text-zinc-400 dark:text-zinc-600 group-hover:text-zinc-900 dark:group-hover:text-zinc-200"}`}
                  >
                    {cat}
                  </span>
                  <Activity
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {filteredSkills.map((skill) => (
                  <motion.div
                    key={skill.id}
                    className="group relative p-5 rounded-[2rem] border border-zinc-200/50 dark:border-zinc-800/50 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-2xl transition-all duration-300 hover:border-blue-500/40"
                  >
                    <div className="relative z-10 flex flex-col gap-5">
                      <div className="flex justify-between items-start">
                        {/* ICON BOX: HIGH-CONTRAST STATIC STAGE */}
                        <div className="w-12 h-12 rounded-2xl bg-white dark:bg-zinc-800 shadow-sm flex items-center justify-center p-3 border border-zinc-200 dark:border-zinc-700/50 transition-all duration-500 group-hover:scale-110 group-hover:bg-blue-50 dark:group-hover:bg-zinc-700 group-hover:shadow-blue-500/10">
                          <img
                            ...
                            onError={(e) => {
                              ...
                              if (!target.dataset.triedBackup) {
                                ...
                                target.src = `https://unpkg.com/simple-icons@v13/icons/${slug}.svg`;
                              } else {
                                target.style.display = "none";
                                const parent = target.parentElement;
                                if (parent)
                                  parent.innerHTML = `<span class="text-[14px] font-black text-blue-600 uppercase">${skill.name.charAt(0)}</span>`;
                              }
                            }}
                          />
                        </div>
                        <div className="flex gap-[2px]">
                          {[1, 2, 3].map((dot) => (
                            <div
                              key={dot}
                              className={`h-1 w-2 md:w-2.5 rounded-full ${dot <= (skill.level === "advanced" ? 3 : skill.level === "intermediate" ? 2 : 1) ? "bg-blue-600 shadow-[0_0_8px_rgba(59,130,246,0.4)]" : "bg-zinc-200 dark:border-zinc-800"}`}
                            />
                          ))}
                        </div>
                      </div>
