...
export default function Home() {
  ...
  const technologies = ["Next.js", "React", "TypeScript", "Tailwind", "Node.js"];
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#050505] text-zinc-900 dark:text-zinc-100 selection:bg-blue-500/30 overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        {/* Soft Ambient Orbs */}
        <motion.div style={{ y: y1 }} className="absolute top-[-10%] right-[-5%] w-[400px] h-[400px] bg-blue-500/[0.06] blur-[100px] rounded-full" />
        <motion.div style={{ y: y2 }} className="absolute bottom-[0%] left-[-5%] w-[500px] h-[500px] bg-purple-500/[0.04] blur-[120px] rounded-full" />
      </div>
          ...
          <div className="lg:col-span-7 space-y-10 text-center lg:text-left order-2 lg:order-1">
            ...
            <motion.div variants={itemVariants} className="space-y-6">
              ...
              <h1 className="text-[12vw] lg:text-[115px] font-black tracking-[-0.07em] leading-[0.85] lg:leading-[0.75] uppercase transition-all">
                Creative <br />
                <span className="text-zinc-200 dark:text-zinc-800 italic font-medium">Architect</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="max-w-xl mx-auto lg:mx-0 space-y-8">
              <p className="text-xl md:text-3xl text-zinc-500 dark:text-zinc-400 font-light leading-snug tracking-tight">
                I am <span className="text-zinc-900 dark:text-white font-bold">{profile.name}</span>. 
                Crafting high-end web systems with <span className="italic underline decoration-blue-500/20 underline-offset-8">absolute precision</span>.
              </p>
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-x-8 gap-y-3">
                {technologies.map((tech) => (
                  <div key={tech} className="flex items-center gap-2 group cursor-default">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-100 transition-colors">{tech}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-4">
              ...
              <Link href="/contact" className="w-full sm:w-auto">
                <Button variant="outline" className="w-full sm:w-auto rounded-none px-12 h-16 border-zinc-200 dark:border-zinc-800 font-black tracking-[0.2em] text-[10px] hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all">
                  GET IN TOUCH
                </Button>
              </Link>
            </motion.div>
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between bg-white/60 dark:bg-zinc-900/40 backdrop-blur-2xl border border-zinc-200/50 dark:border-zinc-800/50 px-8 py-4 rounded-full shadow-xl pointer-events-auto">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest opacity-50">Operational</span>
            </div>
            <div className="hidden sm:flex items-center gap-2">
              <Globe size={11} className="opacity-30" />
              <span className="text-[9px] font-bold uppercase tracking-widest opacity-30">V.2.0.26</span>
            </div>
          </div>
          
          <div className="flex items-center gap-4">
             <div className="h-[1px] w-6 bg-zinc-300 dark:bg-zinc-700" />
             <span className="text-[9px] font-mono opacity-30 uppercase tracking-[0.3em]">Vadodara, Gujarat</span>
          </div>
        </div>
      </motion.div>
